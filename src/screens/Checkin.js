    ...
    timer = async () => {
        ...
        if (orders.length > 0) {
            const ordertime = orders.map(function (item) {
                const id = item.id
                const time = moment(item.orders[0]['order_end_time']).diff(new Date(), 'second')
                return {
                    id,
                    time
                }
            ...
            const timeleft = moment(new Date(time)).diff(new Date(), 'second')
            const last = moment(new Date(time))
            const now = moment(new Date())
            const log = last - now
            const condition = last < now
            const conditionBeta = timeleft == 0
            switch (condition) {
                case true:
            ...
            return (
                <Container>
                                ...
                                renderItem={({ item, index }) => (
                                    ...
                                    >
                                        <View style={[styles.itemContainer, { backgroundColor: item.is_booked == true ? '#4b4b4b' : '#2ecc71' }]}>
                                            ...
                                            <Text style={styles.itemCode}>{item.is_booked == true ? moment(new Date(item.orders[0]['order_end_time'])).diff(new Date(), 'minutes') > 1 ? 
                                                moment(new Date(item.orders[0]['order_end_time'])).diff(new Date(), 'minutes') + ' minutes left'
                                                :
                                                moment(new Date(item.orders[0]['order_end_time'])).diff(new Date(), 'seconds') + ' seconds left':'Available'}</Text>
                                        </View>
                                    </TouchableOpacity>
                    ...
                    <Modal
                        ...
                        onBackButtonPress={() => this.setState({ showModalEdit: '', isLoad: true })}
                        onBackdropPress={() => this.setState({ showModalEdit: '', isLoad: true })}
                    >
                        <View style={[styles.modalGeneral, { marginBottom: 0 }]}>
                            ...
                            <Form style={{ marginTop: 10 }}>
                                ...
                                <View style={{ justifyContent: 'space-between' }}>
                                    <Left>
                                        <Button danger rounded style={{ alignSelf: 'flex-start', marginLeft: -140 }}
                        ...
                        <Container style={styles.modalGeneral}>
                            ...
                            <Form style={{ marginTop: 10 }}>
                                ...
                                <Item style={styles.item}>
                                    <Input style={styles.inputLight}
                                        disabled
                                    ><Text>{isLoadC == false ? moment(dataOrder[0].orders[0]['order_end_time']).diff(new Date(), 'minutes') : null}</Text></Input>
                                </Item>
